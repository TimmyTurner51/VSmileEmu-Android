...
// std::source_location is only available from Android API 30+
#if defined(__ANDROID__) && __ANDROID_API__ < 30
// Fallback for Android API < 30
[[noreturn]] inline void die(const char* msg, const char* file = __builtin_FILE(), int line = __builtin_LINE()) {
  std::clog << file << "(" << line << "): " << msg << std::endl;
  abort();
}
#else
// Use std::source_location on desktop and Android API 30+
